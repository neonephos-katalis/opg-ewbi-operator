...
func (c *k8sClient) GetClientCredentials(ctx context.Context, ClientID string) (ClientCredentials, error) {
	obj, err := c.searchKubernetesObject(&v1beta1.FederationList{}, labels.Set{
		opgLabel(clientIDLabel): ClientID,
	})
	if err != nil {
		return ClientCredentials{}, errors.Wrapf(ErrNotFound, "unkown client ID")
	}
	res, ok := obj.(*v1beta1.Federation)
	if !ok {
		log.Errorf("failed to get federation with %s label '%s': type missmatch, expected %T got %T", clientIDLabel, ClientID, &v1beta1.Federation{}, obj)
		return ClientCredentials{}, ErrInternal
	}
	return ClientCredentials{
		ClientID: res.Spec.FederationData.ClientId,
	}, nil
}
