...
var fileStatusUpdateBackoff = wait.Backoff{
	Steps:    10,
...
// updateFileStatus refetches the object before writing to avoid stale
// resourceVersion conflicts when multiple writes happen in the same or
// overlapping reconciles.
func (r *FileReconciler) updateFileStatus(ctx context.Context, f *v1beta1.File, state v1beta1.FileState) error {
	return retry.RetryOnConflict(fileStatusUpdateBackoff, func() error {
		latest := &v1beta1.File{}
...
func (r *FileReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	if err != nil {
		...
		if upErr := r.updateFileStatus(ctx, &f, v1beta1.FileStateError); upErr != nil {
			log.Error(upErr, errorUpdatingFileStatusMsg)
		}
		return ctrl.Result{}, err
	}

	isRest := IsRestTechnology(string(feder.Spec.Technology))
	...
	if isGuest {
		...
		if f.Status.State == "" {
			log.Info(">>> [File] Initialized new CR state", "state", v1beta1.FileStatePending)
			if upErr := r.updateFileStatus(ctx, &f, v1beta1.FileStatePending); upErr != nil {
				log.Error(upErr, errorUpdatingFileStatusMsg)
			}
		} else {
			if isRest {
