...
func (r *FileReconciler) CreateFile(ctx context.Context, f *v1beta1.File, feder *v1beta1.Federation) error {
	...
	kubeconfigBytes, err := GetKubeconfigFromSecret(ctx, r.Client, feder.Spec.Partner.K8sOptions.FederationSecretName, f.Namespace)
	if err != nil {
...
func (r *FileReconciler) SyncStatusWithHost(ctx context.Context, f *v1beta1.File, feder *v1beta1.Federation) error {
	...
	// Retries update in case of conflict reading always the most recent version of the objsect before re-write the status.
	upErr := retry.RetryOnConflict(retry.DefaultRetry, func() error {
		latest := &v1beta1.File{}
		if err := r.Get(ctx, client.ObjectKeyFromObject(f), latest); err != nil {
			return err
		}
		latest.Status.State = v1beta1.FileState(state)
