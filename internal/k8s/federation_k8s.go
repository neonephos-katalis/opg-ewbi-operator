...
func (r *FederationReconciler) CreateFederation(ctx context.Context, f *v1beta1.Federation) (statusChanged bool, err error) {
	...
	kubeconfigBytes, err := GetKubeconfigFromSecret(ctx, r.Client, f.Spec.Partner.K8sOptions.FederationSecretName, f.Namespace)
	if err != nil {
	...
	fedPatch, err := json.Marshal(map[string]interface{}{
		"spec": map[string]interface{}{
			...
			"guestPartnerCredentials": map[string]interface{}{
				"clientId": f.Spec.GuestPartnerCredentials.ClientId,
			},
		},
	})
	if err != nil {
